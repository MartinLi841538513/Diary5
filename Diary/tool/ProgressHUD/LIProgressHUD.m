...
#error LIProgressHUD is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "LIProgressHUD.h"
#import <QuartzCore/QuartzCore.h>

NSString * const LIProgressHUDDidReceiveTouchEventNotification = @"LIProgressHUDDidReceiveTouchEventNotification";
NSString * const LIProgressHUDWillDisappearNotification = @"LIProgressHUDWillDisappearNotification";
NSString * const LIProgressHUDDidDisappearNotification = @"LIProgressHUDDidDisappearNotification";

NSString * const LIProgressHUDStatusUserInfoKey = @"LIProgressHUDStatusUserInfoKey";

CGFloat LIProgressHUDRingRadius = 14;
CGFloat LIProgressHUDRingThickness = 6;

@interface LIProgressHUD ()

@property (nonatomic, readwrite) LIProgressHUDMaskType maskType;
...
- (void)showProgress:(CGFloat)progress
            ...
            maskType:(LIProgressHUDMaskType)hudMaskType;
...
+ (LIProgressHUD*)sharedView {
    static dispatch_once_t once;
    static LIProgressHUD *sharedView;
...
+ (void)show {
    [[self sharedView] showProgress:-1 status:nil maskType:LIProgressHUDMaskTypeNone];
}

+ (void)showWithStatus:(NSString *)status {
    [[self sharedView] showProgress:-1 status:status maskType:LIProgressHUDMaskTypeNone];
}
...
+ (void)showProgress:(CGFloat)progress {
    [[self sharedView] showProgress:progress status:nil maskType:LIProgressHUDMaskTypeNone];
}

+ (void)showProgress:(CGFloat)progress status:(NSString *)status {
    [[self sharedView] showProgress:progress status:status maskType:LIProgressHUDMaskTypeNone];
}
...
+ (void)showImage:(UIImage *)image status:(NSString *)string {
    NSTimeInterval displayInterval = [[LIProgressHUD sharedView] displayDurationForString:string];
...
- (void)drawRect:(CGRect)rect {
    ...
    switch (self.maskType) {
        ...
        case LIProgressHUDMaskTypeBlack: {
            [[UIColor colorWithWhite:0 alpha:0.5] set];
            CGContextFillRect(context, self.bounds);
            break;
        }
            
        case LIProgressHUDMaskTypeGradient: {
...
- (void)overlayViewDidReceiveTouchEvent {
    [[NSNotificationCenter defaultCenter] postNotificationName:LIProgressHUDDidReceiveTouchEventNotification object:nil];
}

#pragma mark - Master show/dismiss methods
- (void)showProgress:(float)progress status:(NSString*)string maskType:(LIProgressHUDMaskType)hudMaskType {
    ...
    if(self.maskType != LIProgressHUDMaskTypeNone) {
...
- (void)dismiss {
    NSDictionary *notificationUserInfo = (self.stringLabel.text ? @{LIProgressHUDStatusUserInfoKey : self.stringLabel.text} : nil);
    [[NSNotificationCenter defaultCenter] postNotificationName:LIProgressHUDWillDisappearNotification
                                                        object:nil
    ...
    [UIView animateWithDuration:0.15
                     ...
                     completion:^(BOOL finished){
                         if(weakSelf.alpha == 0) {
                             ...
                             [[NSNotificationCenter defaultCenter] postNotificationName:LIProgressHUDDidDisappearNotification
                                                                                 object:nil
                                                                               userInfo:notificationUserInfo];
...
- (CAShapeLayer *)ringLayer {
    if(!_ringLayer) {
        ...
        _ringLayer = [self createRingLayerWithCenter:center radius:LIProgressHUDRingRadius lineWidth:LIProgressHUDRingThickness color:[UIColor whiteColor]];
...
- (CAShapeLayer *)backgroundRingLayer {
    if(!_backgroundRingLayer) {
        CGPoint center = CGPointMake(CGRectGetWidth(hudView.frame)/2, CGRectGetHeight(hudView.frame)/2);
        _backgroundRingLayer = [self createRingLayerWithCenter:center radius:LIProgressHUDRingRadius lineWidth:LIProgressHUDRingThickness color:[UIColor darkGrayColor]];
...
+(void)addActivityView:(UIActivityIndicatorView *)activityView toViewController:(UIViewController *)viewController
{
    ...
    [activityView setFrame:CGRectMake(viewController.view.frame.size.width/2.0-20,viewController.view.frame.size.height/2.0-70, 40, 40)];
...
{
    [activityView setFrame:CGRectMake(view.frame.size.width/2.0-20,view.frame.size.height/2.0-20+view.contentOffset.y, 40, 40)];
    activityView.activityIndicatorViewStyle = UIActivityIndicatorViewStyleWhiteLarge;
    [activityView hidesWhenStopped];
    activityView.color = [UIColor blackColor];
    [activityView startAnimating];
